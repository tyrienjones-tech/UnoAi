...
- [x] **6A.1 Cold clone from origin.** `git clone https://github.com/tyrienjones-tech/UnoAi.git /tmp/unoai-cold-clone` resolved cleanly. Bash-on-Windows mapped `/tmp/unoai-cold-clone` to the cold-clone working directory transparently. Exit 0. No platform-friction at this step.
...
- **Handling rule held throughout.** Doctrine read in-vault only at the operator-environment vault doctrine file (the clean re-paste; original paste-residue version was removed by operator). No doctrine body content reproduced in this entry, in chat, or in Agent Profiles research logs. References use `LESSON-NNN` / `CASE-NNN` identifiers + Builder's own paraphrased UnoAi-side observations only. Lesson titles are not reproduced. Same architecture the vault itself runs on per `Vault/INDUCTION.md` line 52.
- **Doctrine source change documented.** Operator moved doctrine from a lower-severity vault tier (where the `.skill` zip lived) to a higher-severity vault tier (where the clean `.md` lives) during this session. Audit used the clean `.md` at the higher-severity tier. Severity-tier escalation is operator's call; this Builder note documents the source-of-truth location for the audit.
